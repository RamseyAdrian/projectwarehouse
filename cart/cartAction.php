...
if (isset($_REQUEST['action']) && !empty($_REQUEST['action'])) {
    if ($_REQUEST['action'] == 'addToCart' && !empty($_REQUEST['id'])) {
        ...
        $sqlQ = mysqli_query($conn, "SELECT * FROM data_product WHERE product_id = '" . $_GET['id'] . "' ");
        $fo_sqlQ = mysqli_fetch_object($sqlQ);
        ...
        $itemData = array(
            'product_id' => $fo_sqlQ->product_id,
            'product_image' => $fo_sqlQ['product_image'],
            'product_name' => $fo_sqlQ['product_name'],
            'product_price' => $fo_sqlQ['product_price'],
            'qty' => 1
        );
