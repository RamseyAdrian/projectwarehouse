...
$trans = mysqli_query($conn, "SELECT * FROM data_transaction LEFT JOIN data_product USING (product_id) WHERE cart_id = '" . $_SESSION['id_cart'] . "' ");
...
$pdf->setSubject('Surat Pengambilan Barang');
$pdf->setKeywords('Surat Pengambilan Barang');
...
$id_file = $fetch_pesanan['cart_id'];
$id_user = $fetch_pesanan['user_id'];
...
';
$no = 1;
$trans2 = mysqli_query($conn, "SELECT * FROM data_transaction LEFT JOIN data_product USING (product_id) WHERE cart_id = '" . $_SESSION['id_cart'] . "' ");
...
$pdf->Output('SuratPengambilanBarang_ORI_' . $id_file . '_' . $id_user . '.pdf',  'I');
